...
export const useGameStorage = () => {
  ...
  const submitWeek4Result = async (week4Result) => {
    if (storageRef.current) {
      try {
        console.log('Submitting Week 4 result:', week4Result);
        const success = await storageRef.current.submitTeamResult(week4Result);
        if (success) {
          setGameUrl(storageRef.current.getShareableUrl());
          setIsConnected(true);
        ...
        console.error('Error submitting Week 4 result:', error);
        setIsConnected(false);
        return false;
      }
    }
